...
const RecentlyAdded = () => {
  ...
  <div className="text-center text-lg font-bold text-primary">No patients available</div>
) : filteredPatients.length === 0 && search ? (
  <div className="text-center text-lg font-bold text-primary">No patient found for "{search}"</div>
) : (
          filteredPatients.map((data) => (
            <Link key={data.patient_id} to={`${BASE_URL}/${data.patient_id}`} onClick={() => {
    ...
    localStorage.setItem("recentPatients", JSON.stringify(recentPatients.slice(0, 10)));
              ...
              <div className="flex flex-col items-center justify-start py-1 px-2 cursor-pointer">
                 {data?.patient?.image ? <ImageUrl fileKey={data?.patient?.image} className="w-24 h-24 rounded-lg object-cover"  /> :   <img
          src={patientui}
          alt="Patient Icon"
