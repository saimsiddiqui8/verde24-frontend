...
export default function AdminPharmaciesProfile() {
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 my-4 p-4 bg-white rounded-lg shadow-sm">
    ...
    <div className="flex flex-col gap-2 text-sm">
      ...
      <div className="flex items-center gap-4">
        {pharmacyData?.data?.logo ? (
          <ImageUrl fileKey={pharmacyData?.data?.logo} className="w-24 h-24 rounded-md shadow-md" />
        ) : (
    ...
    <div className="flex flex-col gap-2 text-sm">
      ...
      <span className={`px-3 py-1 rounded-full text-xs w-max ${
        pharmacyData?.data?.is_verified
          ? "bg-red-100 text-red-600"
          : "bg-green-100 text-green-600"
      }`}>
        {pharmacyData?.data?.is_verified ? "pharmacy is Banned" : "Not Banned"}
      </span>
    </div>
