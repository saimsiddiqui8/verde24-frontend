...
import { getPharmacyById, updatePharmacyCordinatesById } from "../../../api/apiCalls/pharmacyApi";
import { FIND_PHARMACY_QUERY, UPDATED_PHARMACY_CORDINATES } from "./accountManagement/queries";
...
const PharmacyLocation = () => {
  ...
  const updatePharmacyCordinate = async (data: updatePharmacyCoordinatesFData) => {
    if (!id) return;
...
const handlePharmacyCordinate = ()=>{
  ...
  if (selectedLocation?.lat === data?.latitude && selectedLocation?.lng === data?.longitude && selectedLocation?.address === data?.place_name) {
 ...
 mutate(updatedata)
    ...
    <DashboardSection>
      <div className="flex flex-wrap justify-between items-center gap-4">
        ...
        <div className="flex gap-2">
          <Button title="Edit" className="w-fit" onClick={() => setEdit(true)} />
          {edit && <Button title="Save" className="w-fit" onClick={handlePharmacyCordinate} />}
        </div>
      </div>

      <LoadScript googleMapsApiKey="AIzaSyD_UG0Q5SzKVBFPbxwfs1q9dRjnxsmhQBo" libraries={libraries}>
  ...
  <div className="text-sm my-2">
    ...
    <div className="flex items-center gap-3 mt-2">
      <div className="rounded-full bg-pink-50 p-2 flex items-center justify-center">
        <img className="w-12 h-6" src={locationicon} alt="Location-icon" />
      </div>

      <div>
        <p className="font-medium text-sm">Current Location</p>
        <p className="text-primary text-sm">{selectedLocation.address}</p>
      </div>
    </div>
  </div>
) : (
  <p>No location selected</p>
          ...
          <div className={`relative h-[400px] rounded-lg shadow-md w-full ${!edit ? "pointer-events-none opacity-70" : ""}`}>
            <GoogleMap
