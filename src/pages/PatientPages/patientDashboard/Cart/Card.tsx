...
const Card = () => {
  ...
  const getCard = async () => {
    if (!id) return;
    const response = await FindCardById(CARD_BY_PATIENT_ID, { patientId: id });
    if (!response) {
  ...
  const total = data?.reduce(
    (total: number, item: CartItem) => total + item.labTest.price,
    0,
  );
    ...
    <DashboardSection title="Your Carts">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {(data ?? [])?.length > 0 ? (
          ...
          <p className="text-start text-primary text-xl font-semibold mt-5">
            No carts added
          </p>
        )}
      </div>
      ...
      {data?.length > 0 && (
        <div className="mt-8 border-2 border-primary p-6 flex flex-col items-center">
          ...
          <Button
            ...
            secondary={true}
            className="rounded-xl w-52 text-lg p-3"
          />
        </div>
      )}
      <Toaster />
