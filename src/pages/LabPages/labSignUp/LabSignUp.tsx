...
export default function LabSignUp() {
      ...
      async (position) => {
        ...
        try {
          ...
          if (data.status === "OK" && data.results.length > 0) {
            ...
            notifyFailure("Address not found");
          }
        } catch (error) {
          notifyFailure("Error fetching address: " + error);
        }
      },
      (error) => {
        notifyFailure(
  ...
  const checkDuplicateEmail = async () => {
    return findLabByEmail(EXISTING_LAB_QUERY, {
      email: getValues("email"),
    });
  };

  const sendOTP = async () => {
    return sendLabOTP(SEND_OTP_QUERY, {
      email: getValues("email"),
      role: "lab",
    });
  };
  const labEmail = useQuery({
    queryFn: checkDuplicateEmail,
    enabled: false,
  });
  const verifyOTP = async () => {
    return verifyLabOTP(VERIFY_OTP_QUERY, {
      email: getValues("email"),
      role: "lab",
  ...
  const { data, mutate } = useMutation(createNewLab);

  const verifyOTPData = useQuery({
    queryFn: verifyOTP,
    enabled: false,
  });
  const labOTPData = useQuery({
    queryFn: sendOTP,
    enabled: false,
  });
  const sendOtp = async () => {
    dispatch(loadingStart());
    const otp = await labOTPData.refetch();
  ...
  const handleValidation = async () => {
    dispatch(loadingStart());
    const email = await labEmail.refetch();
    if (email?.data?.id) {
  ...
  const onSubmit = async () => {
    ...
    const place_name = getValues("place_name");
