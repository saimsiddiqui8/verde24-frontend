...
import { LAB_TOKEN_QUERY } from "./queries";
...
export default function LabSignIn() {
  ...
  const handleLogin = async (data: Inputs) => {
    ...
    const tokenRes = await getLabToken(LAB_TOKEN_QUERY, data);
    dispatch(loadingEnd());
    if (tokenRes?.token) {
      const userData = { ...tokenRes, role: USER_ROLES.lab };
  ...
  return (
    <main className="grid grid-cols-12 items-center gap-8 mb-6 px-4 md:px-8">
        ...
        <div className="w-full justify-self-center">
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="text-primary my-3 flex justify-between items-center">
              <h3 className="text-2xl md:text-3xl font-bold">Lab Login</h3>
            ...
            <button className="form-btn my-3">Lab Login</button>
          </form>
      ...
      <section className="col-span-12 md:col-span-6 lg:col-span-4 order-1 md:order-2">
        <img src={image} alt="Lab Image" className="w-full h-auto" />
      </section>
      <Toaster />
    </main>
  );
